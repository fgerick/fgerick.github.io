...
# ╔═╡ 56062c50-e622-11ef-291b-9bc6ca403506
using WGLMakie, Bonito

# ╔═╡ 8026c323-0b42-4642-b160-2abad4087dea
begin
	using Limace
...
# ╔═╡ a0f5428b-b148-4227-9ab4-19265ae8f137
begin
	
	@inline function cart2sph(R)
		x,y,z = R
		r = sqrt(x^2+y^2+z^2)
		θ = atan(sqrt(x^2+y^2),z)
		ϕ = atan(y,x)
		return SVector(r,θ,ϕ)
	end
	
	@inline function sph2cart(R)
		r,θ,ϕ = R
		return SVector(r*cos(ϕ)*sin(θ), r*sin(ϕ)*sin(θ), r*cos(θ))
	end
	
	@inline function sph2cartvec(u,R)
		r,θ,ϕ = R
		ur,uθ,uϕ = u
	...
	function solvetracing!(integrator; rmax=30, maxiter=100, r1=1.0)
	
	    while norm(integrator.sol[end])<rmax
	        if (length(integrator.sol.t)>maxiter)  && (norm(integrator.sol[end])<=r1)
	            return
	        end
	        if (length(integrator.sol.t)>100)  && (norm(first(integrator.sol)-last(integrator.sol))<r1/2)
	            return
	        end
	...
	function f(Cr, u, t)
		r = cart2sph(Cr)
		dr = real(sph2cartvec(discretize(u,r...),r))
		# dr = ufunc(r)
		return dr
	end
	
	function traceparticle(u,r0; solver=Vern9(), tspan = (0.0,Inf), kwargs...)
	    prob=ODEProblem(f,r0,tspan,u);
	    integrator = init(prob,solver; dense=true)
		# integrator.dt = 0.01
	    solvetracing!(integrator; kwargs...)
	    return integrator.sol.u
	end
	
	function drawsphere(R=1.0)
		ϕ = rand()*2π
		cosθ = 2rand()-1
		θ = acos(cosθ)
		r = R * ∛rand()
...
# ╔═╡ 36c771de-905e-414b-a389-936c4605e290
begin
	function spheregrid(n,r,phimax)
	    θ = range(0,pi,length=n)
	    φ = range(0,phimax,length=n)
	    # φ = [(0:2n-2)/(2n-1);2]
	    x__ = [r*cos(φ)*sin(θ) for θ in θ, φ in φ]
	    y__ = [r*sin(φ)*sin(θ) for θ in θ, φ in φ]
	    z__ = [r*cos(θ) for θ in θ, φ in φ]
	    return x__,y__,z__
	end
	
	function setupsurfsphere(n,br,phimax=3pi/2)
	
	    x__,y__,z__ = spheregrid(n,1.0,phimax)
	    br_surf = map((x,y,z)->br(x,y,z),x__,y__,z__)
	    br_surf_buffer = real.(br_surf)
	    brsurf_ = Node{Matrix{Float64}}(br_surf_buffer);
	    return br_surf, brsurf_, x__, y__, z__ 
	end
end

# ╔═╡ 4130d0c6-3d21-474e-bc34-80d521823157
begin
	function randcomponent(L)
		l = rand(1:L)
		m = rand(0:l)
		n = rand(1:L)
		return BasisElement(Basis{Insulating}, rand((Poloidal,Toroidal)), (l,m,n), randn(ComplexF64)/l^4)
	end
	
	function randcomponents(L)
		return [BasisElement(Basis{Insulating}, PT, (l,m,n), (1+randn(ComplexF64))/(l^4+(m+2)^4+n^4)) for l in 1:L for m in 0:l for n in 1:(L-l)÷2 for PT in (Poloidal,Toroidal)]
	end
	
	
end

# ╔═╡ 5059a68e-92f8-4cde-a911-4e0f2719d2bf
earth_img = load(joinpath("/Users/felixger/Downloads/","earth_daylight.jpg"));

# ╔═╡ e7882e96-5449-4a60-a691-b2731c485ee6
B0 = randcomponents(6);

# ╔═╡ c9ccb7a9-4e59-4115-b96a-f0d580dae06c
Random.seed!(10)

# ╔═╡ c4d075c5-f88c-43db-81e2-724c483783a6
np = 20

# ╔═╡ 9d028be8-878a-48e4-b3a1-7a577adc4717
traced = [traceparticle(rand((-1,1)).*B0, drawsphere(0.99), rmax=50, solver=OwrenZen3(), maxiter=500) for _ in 1:np];

# ╔═╡ d054ce63-c34f-4d40-94cd-fb44f3bcec24
__theme = CatppuccinMakieThemes.mocha;

# ╔═╡ 22271541-7c50-42a8-9928-2e69d4c051d9
_theme = CatppuccinMakieThemes.theme(__theme);

# ╔═╡ 84774b18-9167-4b3e-a81b-4436beeaa8f2
WGLMakie.set_theme!(_theme)

# ╔═╡ e60319ae-ce81-4bbe-968e-7f168d99ee7b
fig = let 
	npx = 400
	Rc = 3486
	R0 = 6371
	...
	# solvers: OwrenZen3, Vern8

	# ax = GLMakie.Scene()
	# ax = Axis(fig[1,1]) #; aspect=(1, 1, 1))
	fig = Figure(size=(npx,npx))
	ax = LScene(fig[1,1], show_axis=false)
	_cc = Makie.Camera3D(ax.scene, 
			# projectiontype = Makie.Orthographic, 
			center=false,
			eyeposition = [1,-3,1.7]*2.0, 
			clipping_mode = :view_relative, near=1e-2, far=10.0)

			# 
			update_cam!(ax.scene) 
			# ; clipping_mode = :static, near=1e-9, far=10.0)
	# update_cam!(ax.scene)
	# x__in, y__in,z__in = spheregrid(100,0.35,2pi)
	# surface!(ax, x__in,y__in,z__in, color=(:black,0.1), colormap=:Greys, transparency=false)
	
	sphere = Sphere(Point3f(0), 1.0f0)
	# x__in, y__in,z__in = spheregrid(50,1.0,3pi/2)
	# points = vec([Point3f(xv, yv, zv) for (xv, yv, zv) in zip(x__in, y__in,z__in)])
	# _faces = decompose(WGLMakie.GeometryBasics.QuadFace{WGLMakie.GeometryBasics.GLIndex}, WGLMakie.GeometryBasics.Tessellation(Rect(0, 0, 1, 1), size(z__in)))
	# gb_mesh = WGLMakie.GeometryBasics.Mesh(points,_faces) #; uv = uv_buff, normal = _normals)
	# @show extrema(z__in)
	mesh!(ax, sphere, color=:grey,backlight = 1, clip_planes = clip_planes)
	# mesh!(ax,gb_mesh, color=:grey, backlight=1)
	# surface!(ax, x__in,y__in,z__in, color=(z__in.+0.5)./2,colormap=Reverse(:Greys),colorrange=(0,1),transparency=false)

	# x__in, y__in,z__in = spheregrid(200,R0/Rc,3pi/2)
	# surface!(ax, x__in*0.999,y__in*0.999,z__in*0.999, color=fill(0.0,size(z__in)...), colormap=:Greys, colorrange=(1,0), transparency=false)
	
	# surface!(ax, x__in,y__in,z__in, transparency=false, color=earth_img[:,1:3*end÷4])
	_cmap = __theme == CatppuccinMakieThemes.latte ? :devon : Reverse(:devon)
	maxB = 0.0
	for trace in traced
		for r in trace
			maxB = max(maxB,norm(discretize(B0,cart2sph(r)...)))
		end
	end
	for trace in traced
		absB = [norm(discretize(B0,cart2sph(r)...)) for r in trace]
		lines!(ax, getindex.(trace,1), getindex.(trace,2), getindex.(trace,3), color=absB, colorrange=(0,maxB), colormap=_cmap, linewidth=3)
	end
	
	# xlims!(-3,3)
	# ylims!(-3,3)
	# zlims!(-3,3)
	# save("/Users/felixger/Downloads/rspa_cover.jpg",fig)
	fig
end



# ╔═╡ 971f37e8-1db7-43c3-80de-8a32606d7799
# _theme

# ╔═╡ e79fa486-cb09-4889-af27-ca82c35eb82a
app = Bonito.App(; title="Magnetic field visualization") do
	Bonito.DOM.div(Bonito.Styling, Bonito.TailwindCSS, fig)
end;

# ╔═╡ 298dc5ce-3656-4b0b-87ab-f271d73e0cf0
# open("/Users/felixger/Repositories/fgerick.github.io/_assets/figure_magfield_$(__theme.name).html","w") do f
# 	println(f,Bonito.DOM.div(Bonito.Styling, Bonito.TailwindCSS, fig))
# end

# ╔═╡ d840c09c-e657-42a3-86f5-40779202b6f9
Bonito.export_static("/Users/felixger/Repositories/fgerick.github.io/_assets/figure_magfield_$(__theme.name).html", app)

# ╔═╡ 74d1eea2-b8b9-47f8-933a-01d23c3f1f02
# Bonito.export_standalone("/Users/felixger/Repositories/fgerick.github.io/_assets/figure_magfield_$(__theme.name).html", app)

# ╔═╡ 1955dabe-738d-4f94-8016-e61e6406592a
# let
# 	f=Figure()
# 	ax=LScene(f[1,1])
# 	scatter!(ax,randn(30),randn(30),randn(30))
# 	f
# end
...
CatppuccinMakieThemes = "c37c50e1-20da-4a65-b669-80e6e9abe7a3"
Images = "916415d5-f1e6-5110-898d-aaa5f9f070e0"
JSServe = "824d6782-a2ef-11e9-3a09-e5662e0c26f9"
Limace = "586a1cab-6cd8-47f7-be84-7810086014ba"
...
manifest_format = "2.0"
project_hash = "d8b5d55c157ceb7cf8c9912e9e0cef5ac5688640"
...
[[deps.CatppuccinMakieThemes]]
deps = ["ColorTypes", "FixedPointNumbers", "MakieCore"]
git-tree-sha1 = "7f65f4c0dd46d4186c24e07e38d3e037b5f484a4"
repo-rev = "main"
repo-url = "https://github.com/fgerick/CatppuccinMakieThemes.jl"
uuid = "c37c50e1-20da-4a65-b669-80e6e9abe7a3"
version = "1.0.0-DEV"
...
git-tree-sha1 = "fd1517018220b5dd91ea1ed9ba27ccc95345fbcb"
...
[[deps.Limace]]
deps = ["ArnoldiMethod", "DocStringExtensions", "FastGaussQuadrature", "ForwardDiff", "LinearAlgebra", "LinearMaps", "Random", "Reexport", "SHTns", "SparseArrays", "SpecialFunctions", "StaticArrays", "Statistics", "WignerSymbols"]
git-tree-sha1 = "e624de485150115c5679b580280bc3d76dd7fdf3"
repo-rev = "main"
repo-url = "https://github.com/fgerick/Limace.jl"
uuid = "586a1cab-6cd8-47f7-be84-7810086014ba"
version = "0.1.0"
...
[[deps.SHTns]]
deps = ["SHTns_jll"]
git-tree-sha1 = "c99cfdee4d6094bbd1ebbb7d0cfc676be4509c86"
uuid = "5dc4ea12-f621-4038-8458-a22eae100703"
version = "0.2.0"

[[deps.SHTns_jll]]
deps = ["Artifacts", "CUDA_Runtime_jll", "CompilerSupportLibraries_jll", "FFTW_jll", "JLLWrappers", "LLVMOpenMP_jll", "LazyArtifacts", "Libdl", "TOML"]
git-tree-sha1 = "1bf4428e3bc8d2e14309167bfb51e5fa5c816ae8"
uuid = "daf09cc5-9ab3-509e-9618-0b89086eb825"
version = "3.7.0+0"
